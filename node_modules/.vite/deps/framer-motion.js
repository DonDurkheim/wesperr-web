import {
  require_jsx_runtime
} from "./chunk-LGKT6QQ5.js";
import {
  __commonJS,
  __export,
  __toESM,
  require_react
} from "./chunk-OWUOI2EO.js";
  ...
  if (key === "pathOffset") {
    ...
    if (!state.get("stroke-dasharray")) {
      state.set("stroke-dasharray", new MotionValue("1 1"), () => {
        ...
        element.setAttribute("stroke-dasharray", `${toPx(pathLength)} ${toPx(pathSpacing != null ? pathSpacing : 1 - Number(pathLength))}`);
      });
    }
...
function createRendererMotionComponent({ preloadedFeatures, createVisualElement, useRender, useVisualState: useVisualState2, Component: Component3 }) {
  ...
  MotionComponent.displayName = `motion.${typeof Component3 === "string" ? Component3 : `create(${(_b = (_a = Component3.displayName) != null ? _a : Component3.name) != null ? _b : ""})`}`;
...
function getTimeline({ source, container, ...options }) {
  ...
  if (source)
    container = source;
  const containerCache = (_a = timelineCache.get(container)) != null ? _a : /* @__PURE__ */ new Map();
  timelineCache.set(container, containerCache);
  const targetKey = (_b = options.target) != null ? _b : "self";
  const targetCache = (_c = containerCache.get(targetKey)) != null ? _c : {};
  const axisKey = axis + ((_d = options.offset) != null ? _d : []).join(",");
